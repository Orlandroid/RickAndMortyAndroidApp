...
data class CharacterDetailUiState(
    ...
    @param:ColorRes val imageBorderColor: Color = Unknown
)
        ...
        when (event) {
            ...
            is CharacterDetailEvents.OnCharacterClicked -> {
                ...
                if (idCharacter != event.characterId) {
                    viewModelScope.launch {
                        _effects.send(CharacterDetailEffects.NavigateToCharacterDetail(event.characterId))
                    }
                }
            }
            ...
            CharacterDetailEvents.OnBack -> {
                viewModelScope.launch {
                    _effects.send(CharacterDetailEffects.NavigateBack)
                }
            }
        }
    }
