package com.example.characters.screens.characters
        ...
        when (event) {
            is CharacterEvents.OnCharacterClicked -> {
                viewModelScope.launch {
                    _effects.send(CharacterEffects.NavigateToCharacterDetail(event.characterId))
                }
            }

            CharacterEvents.OnSearchClicked -> {
                viewModelScope.launch {
                    _effects.send(CharacterEffects.NavigateToSearchScreen)
                }
            }
        }
    }
